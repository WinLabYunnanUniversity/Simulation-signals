%循环1000次，获取数据WCDMA_1429—1452MHz
tic; 
Fs=15.36e6;
test = ori_sig(1+51200*9:51200+51200*10,1);
...
for k=1:1
    ...
    % plot(new_sig)
    xi=linspace(1429,1452,len_sig);
    figure(1);
    plot(xi,new_sig);
    % title('1429—1452MHz频谱图');
    xlabel('Frequency (MHz)')
    ylabel('dBm');
    ylim([-120,20]);
    xlim([1429,1452]);
    set(gca,'xtick',1429:4.6:1452)
    sig(k,:)=new_sig;%数据拼接
    k1=k-1;
%     filename=['image\6.28mp4\WCDMA\',num2str(k1),'.png'];
%     saveas(gcf,filename)
end
% save('dataset\WCDMA1.mat','sig');
